...
(function () {
  ...
  function svgMarkup(icon, sw) {
    return '<svg xmlns="' + SVGNS + '" width="24" height="24" viewBox="0 0 24 24" fill="none" ' +
      'stroke="currentColor" stroke-width="' + sw + '" stroke-linecap="round" stroke-linejoin="round">\n  ' +
      icon.b.replace(/></g, ">\n  <") + "\n</svg>";
  }
  function pascal(name) {
    return name.split(/[-_]/).map(function (p) { return p.charAt(0).toUpperCase() + p.slice(1); }).join("");
  }
  function jsxMarkup(icon, sw) {
    var jbody = icon.b.replace(/></g, ">\n    <");
    return "const " + pascal(icon.n) + "Icon = (props) => (\n" +
      '  <svg xmlns="' + SVGNS + '" width={24} height={24} viewBox="0 0 24 24" fill="none"\n' +
      '    stroke="currentColor" strokeWidth={' + sw + '} strokeLinecap="round" strokeLinejoin="round" {...props}>\n    ' +
      jbody + "\n  </svg>\n);";
  }
  ...
  function copy(text, label, name) {
    var done = function () { ping(label, name); };
  ...
  function render() {
    ...
    order.forEach(function (cat) {
      var items = list.filter(function (i) { return i.c === cat; });
      if (!items.length) return;

      var head = document.createElement("div");
      head.className = "group-head";
      head.innerHTML = '<h2>' + cat + '</h2><span class="line"></span><span class="gc">' + items.length + '</span>';
      ...
      items.forEach(function (icon) {
        ...
        quick.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round"><rect x="8.5" y="8.5" width="11" height="11" rx="2"/><path d="M15.5 8.5V6.5a2 2 0 0 0-2-2h-7a2 2 0 0 0-2 2v7a2 2 0 0 0 2 2h2"/></svg>';
        ...
        cell.appendChild(nm);
        cell.addEventListener("click", function () { openDrawer(icon); });
        cell.addEventListener("keydown", function (ev) {
          if (ev.key === "Enter" || ev.key === " ") { ev.preventDefault(); openDrawer(icon); }
        });
  ...
  function curWeight() { return parseFloat(swEl.value).toFixed(2).replace(/\.?0+$/, "") || "1.75"; }
  ...
  function openDrawer(icon) {
    ...
    dKeys.innerHTML = "";
    icon.k.split(/\s+/).slice(0, 10).forEach(function (k) {
      var t = document.createElement("span");
      t.className = "dkey"; t.textContent = k;
      dKeys.appendChild(t);
    });
  ...
  /* ——— Controls ——————————————————————————————————— */
  swEl.addEventListener("input", function () { root.style.setProperty("--sw", swEl.value); });
  szEl.addEventListener("input", function () { root.style.setProperty("--isz", szEl.value + "px"); });
  ...
  q.addEventListener("input", function () {
    clearTimeout(qTimer);
    qTimer = setTimeout(function () { query = q.value.trim().toLowerCase(); render(); }, 90);
  });
